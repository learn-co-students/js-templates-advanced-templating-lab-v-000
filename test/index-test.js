describe("Handlebars Templates Lab", function() {
  describe("page", function() {
    ...
    it("has a recipe form", function() {
      ...
      expect(recipeForm).toExist(
        "Must provide a form with an id of 'recipe-form'"
      );
  ...
  describe("templates", function() {
    it("has a recipe template", function() {
      ...
      expect(recipeTemplate).toExist(
        "Must provide a template with an id of 'recipe-template'"
      );
      expect(recipeTemplate.type).toBe(
        "text/x-handlebars-template",
        "Template must be of type text/x-handlebars-template"
      );
      expect(recipeTemplate.innerHTML).toMatch(/{{\s?name\s?}}/);
      expect(recipeTemplate.innerHTML).toMatch(
        /<a.*displayEditForm().*>Edit Recipe<\/a>/,
        "Template must have an 'Edit Recipe' link that calls 'displayEditForm()'"
      );
      expect(recipeTemplate.innerHTML).toMatch(
        /{{>\s?recipeDetailsPartial\s?}}/,
        "Template must render the recipeDetailsPartial"
      );
    });
    it("has a recipe details partial template", function() {
      var recipeDetailsPartial = document.getElementById(
        "recipe-details-partial"
      );
      expect(recipeDetailsPartial).toExist(
        "Must provide a template with an id of 'recipe-details-partial'"
      );
      expect(recipeDetailsPartial.type).toBe(
        "text/x-handlebars-template",
        "Template must be of type text/x-handlebars-template"
      );
      expect(recipeDetailsPartial.innerHTML).toMatch(/{{\s?description\s?}}/);
      expect(recipeDetailsPartial.innerHTML).toMatch(
        /{{\s?#each ingredients\s?}}/
      );
      expect(recipeDetailsPartial.innerHTML).toMatch(
        /{{\s?displayIngredient\s?}}/,
        "Template must make use of displayIngredient custom helper inside the #each block helper"
      );
    });
    it("has a recipe form template", function() {
      var recipeFormTemplate = document.getElementById("recipe-form-template");
      expect(recipeFormTemplate).toExist(
        "Must provide a template with an id of 'recipe-form-template'"
      );
      expect(recipeFormTemplate.type).toBe(
        "text/x-handlebars-template",
        "Template must be of type text/x-handlebars-template"
      );
    });
